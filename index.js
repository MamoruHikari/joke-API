...
app.get("/jokes/:id", (req, res) => {
  ...
  const joke = jokes.find(j => j.id === urlID);
  res.json(joke);
});
...
app.get("/filter", (req, res) => {
  ...
  const filteredJokes = jokes.filter(j => j.jokeType === queryType);
...
app.patch("/jokes/:id", (req, res) => {
  const urlID = parseInt(req.params.id, 10);
  const jokeToPatch = jokes.find(j => j.id === urlID);

  if (req.body.text) {
    jokeToPatch.jokeText = req.body.text;
  }
  
  if (req.body.type) {
    jokeToPatch.jokeType = req.body.type;
  }
...
app.delete("/jokes/:id", (req, res) => {
  const urlID = parseInt(req.params.id, 10);
  const jokeIndex = jokes.findIndex(j => j.id === urlID);
